...
### 退避の優先順位ルール（確定仕様・2026-07-22）

ユーザー指示（複数回反復・最優先）。「メイン画面を空にする」および切替時の退避は、この順で決める：

1. **サブ画面を指定されたセットの窓** → その指定サブ画面へ退避する。そのサブ画面は以後「使用中」。
2. **サブ指定のない登録窓** → **使用中のサブ画面には絶対に入れない**（重ねない）。サブ画面のモニター
   *以外*の退避先（非メイン・非サブ・非除外モニター）へ「ガンガン」詰める。入り切らなければ
   そのモニターを 2分割 → 4分割 して**入れられるだけ入れる**。
3. どの退避先にも入り切らない窓を、**そのとき初めて最小化**する。未登録（どのセットにも属さない）窓は最小化。

→ 実装上のキモ: 自動割当（`parking_allocator::allocate_parking`）の対象モニターから、
   **サブ画面が使うモニターを除外**する（`sub_screen_monitor_ids`）。これによりサブは指定セット専用に
   保たれ、非指定の窓はサブ以外の画面にだけ詰められる。
