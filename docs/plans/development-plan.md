...
- **退避先固定（`ParkingPolicy::Fixed`＋`FixedParkingSlot`のUI）**: 登録画面で退避先を「自動／固定」で選択。
  固定時はメイン以外のモニター一覧から退避先を選び、分割（1/2/4）とセルを指定。登録時に`FixedParkingSlot`を生成して
  `parking_policy`を紐付ける。バリデーション失敗時はワークセットと枠を両方ロールバック。
